...
import { SalesTransaction } from '@/pages/Dashboard/Reports/SalesReportPage';
...
const SalesReportPrint: React.FC<SalesReportPrintProps> = ({ data, range }) => {
        ...
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr className="border-b">
              ...
              <th className="text-left p-2 font-semibold">No. Transaksi</th>
            ...
            {data.map(t => {
              ...
              return (
                <tr key={t.id} className="border-b">
                  ...
                  <td className="p-2 font-mono">{t.transaction_id_display}</td>
                  <td className="p-2">{t.customer_name_cache || 'Umum'}</td>
                  <td className="text-right p-2">{formatCurrency(t.total)}</td>
