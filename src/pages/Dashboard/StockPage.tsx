...
import { PlusCircle, Search, Edit, Trash2, RefreshCw, PlusSquare, Barcode as BarcodeIcon } from "lucide-react";
...
const StockPage = () => {
  ...
  // Effect to find item by barcode when scanning in "Add Stock" dialog
  useEffect(() => {
    if (barcodeToSearch.length > 5) { // Trigger search on a reasonable length
      const loadingToast = showLoading("Mencari barang...");
      const foundItem = stockData.find(item => item.barcode === barcodeToSearch);
      dismissToast(loadingToast);
      if (foundItem) {
        setStockToAdd(prev => ({ ...prev, itemId: foundItem.id }));
        showSuccess(`Barang ditemukan: ${foundItem.name}`);
        setBarcodeToSearch(""); // Clear input for next scan
      } else {
        showError("Barcode tidak ditemukan.");
      }
    }
  }, [barcodeToSearch, stockData]);
                  ...
                  <form onSubmit={handleAddStockSubmit}>
                    <div className="grid gap-4 py-4">
                      <div className="grid grid-cols-4 items-center gap-4">
                        <Label htmlFor="barcode-search" className="text-right">Scan Barcode</Label>
                        <div className="col-span-3 relative">
                          <BarcodeIcon className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
                          <Input id="barcode-search" placeholder="Scan di sini..." className="pl-10" value={barcodeToSearch} onChange={(e) => setBarcodeToSearch(e.target.value)} />
                        </div>
                      </div>
                      <div className="grid grid-cols-4 items-center gap-4">
                ...
                <DialogContent className="sm:max-w-[425px]">
                  ...
                  <form onSubmit={handleAddItemSubmit}>
                    <div className="grid gap-4 py-4">
                      ...
                      <div className="grid grid-cols-4 items-center gap-4"><Label htmlFor="resellerPrice" className="text-right">Harga Seller</Label><Input id="resellerPrice" name="resellerPrice" type="number" placeholder="Rp 0" className="col-span-3" value={newItem.resellerPrice} onChange={handleNewItemChange} /></div>
                    </div>
                    <DialogFooter>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-right">Harga Seller</TableHead>
